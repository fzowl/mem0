...
# Total tokens allowed in a single request, per model (from the Voyage AI by MongoDB
# docs at https://docs.voyageai.com/docs/embeddings). Used to build token-aware batches.
...
class VoyageEmbedding(EmbeddingBase):
    def __init__(self, config: Optional[BaseEmbedderConfig] = None):
        ...
        # We call the official `voyageai` SDK, so the MongoDB API redirection is handled
        # for us: the SDK's `get_default_base_url()` sends keys with an "al-" prefix to
        # https://ai.mongodb.com/v1 and all other keys to https://api.voyageai.com/v1.
        # See https://github.com/voyage-ai/voyageai-python/blob/main/voyageai/util.py.
    ...
    def embed(self, text, memory_action: Optional[Literal["add", "search", "update"]] = None):
        """
        Get the embedding for the given text using Voyage AI by MongoDB.
